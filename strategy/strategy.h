...
struct strategy_s
{
  ...
  /** A function which returns the direction chosen by the strategy.
 The first parameter is a pointer to the current strategy
 (useless for memoryless strategies). 
 The second one is the current grid. */
  void (*free_strategy) (strategy);
  /** Frees any resources allocated during the strategy's creation (<name>, <mem>, ...). */
};
